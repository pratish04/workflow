        ...
        bcrypt.hash(req.body.password, 10, (err, hash)=>{
            if(err)
                ...
                db.query("INSERT INTO users (firstName, lastName, email, password) VALUES(?, ?, ?, ?);", 
                    [req.body.firstName, req.body.lastName, req.body.email, hash], (error, results)=>{
                    if(error){
                        return reject(error);
                    }
                    else{
...
router.post("/", async(req, res)=>{
    try{
        ...
        const token = jwt.sign({ userId: result3[0].userId }, process.env.ACCESS_TOKEN_SECRET);
        res.cookie("accessToken", token, {
            httpOnly: true,
            secure: process.env.NODE_ENV === "production",
        }).send({registrationSuccessful: true, message: "USER REGISTRATION SUCCESSFUL!"});
